...
import type { PostStatus, PostWithTags } from './types';
...
const normalizeTagList = (values: unknown[]) => {
...
const parseTagPatch = (current: string[], patch: CodexTagPatch) => {
...
const requireCodexToken = (request: Request, env: Env) => {
...
const logCodexAudit = (method: string, path: string, postId?: string | null) => {
  const id = postId ?? '-';
  console.log(
...
const serializePost = (post: PostWithTags) => {
  return {
    id: post.id,
    slug: post.slug,
    title: post.title,
